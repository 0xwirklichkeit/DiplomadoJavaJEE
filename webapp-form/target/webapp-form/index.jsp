...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-0evhe/X+RZYKIZDRvuzKMRqM+Or
</head>
<body>
...
<form action="/webapp-form/registro" method="post">
    ...
    <div class="row mb-3">
        ...
        <%
        if(errores != null && errores.getNamedkeys().contains("username")){
            out.println("<div class='row mb-3 alert alert-danger col-sm-4' style='color: blue;'>" + errores.get("username") + "</div>");
        }
        %>
    <div class="row mb-3">
        <label for="password" class="col-form-label col-sm-2">Passwords</label>
        <div class="col-sm-4"><input type="password" name="password" id="password" class="form-control"></div>
            <%
        if(errores != null && errores.containskey("password")){
            out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>"+ errores.get("password") + "</small>");
        }
        %>
    </div>
    <div class="row mb-3">
    ...
    <%
        if(errores != null && errores.containskey("email")){
            out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>"; errores.get("email") + "</small>");
        }
        %>
    </div>
    <div class="row mb-3">
        ...
        <div class="col-sm-4">
            <select name="pais" id="pais" class="form-select">
                ...
                <option value="CL" S(param.pais.equals("CL")? "selected": ""}>Chile</option>
                <option value="AR" ${param.pais.equals("AR")? "selected": ""}>Argentina</option>
                <option value="PE" ${param.pais.equals("PE")? "selected": ""}>Perú</option>
                <option value="CO" S{param.pais.equals("CO")? "selected": ""}>Colombia</option>
                <option value="VE" S{param.pais.equals("VE")? "selected": ""}Venezuela</option
             </select>
        </div>
        <%
            if (errores != null && errores.containskey("pais")){
                out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>" + errores.get("pals") + "</small>");
            }
            %>
    </div>
    <div class="row mb-3">
        ...
        <%
            if(errores != null && errores.containskey("lenguajes")){
                out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>"+ errores.get("lenguajes") + "</small>");
            }
            %>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="form-check col-sm-2">
            <input type="checkbox" name="roles" value="ROLE ADMIN" ${paramvalues.roles.stream().anyMatch(v->v.equals("ROLE_ADMIN")).get()?"checked":""} class="form-check-input">
        ...
        <%
            if(errores != null && errores.containsKey("roles")){
                out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>" + errores.get("roles") + "</small>");
    </div>
    <div class="row mb-3">
        ...
        <div class="form-check col sm-2">
            <input type="radio" name="idiona" value="fr" class="form-check-input" ${param.idioma.equals("fr")? "checked":""}>
            <label class="form-check-label"Frances</label>
        </div>
        <%
            if(errores != null && errores.containsKey("idioma")) {
                out.println("<small class='alert alert-danger col-sm-4' style='color: blue;'>" + errores.get("idioma") + "</small>");
            }
            %>
    </div>
    <div class="row mb-3">
